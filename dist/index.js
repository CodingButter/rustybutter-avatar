...
let expressions = [];
try {
    const expressionsPath = path_1.default.join(__dirname, '../public/expressions.json');
    console.error(`[Debug] Loading expressions from: ${expressionsPath}`);
    const expressionsData = fs_1.default.readFileSync(expressionsPath, 'utf8');
...
let batchExpressionsState = null;
// Callback for MCP to update our state
const handleExpressionUpdate = (name, state) => {
...
const handleBatchExpressionsUpdate = (batchExpressions) => {
    ...
    if (validActions.length === 1 && !batchExpressions.loop) {
        ...
        batchExpressionsState = {
            ...
            batchId: batchExpressions.batchId || (0, uuid_1.v4)() // Generate a new ID if not provided
        };
...
app.get('/api/set-expression', (req, res) => {
    ...
    if (pos_x !== undefined) {
        try {
            const parsedPosX = parseInt(pos_x, 10);
    ...
    if (pos_y !== undefined) {
        try {
            const parsedPosY = parseInt(pos_y, 10);
...
app.listen(PORT, () => {
    ...
    if (isMcpMode) {
        ...
        (0, mcp_server_1.startMcpServer)(expressions, expressionMap, handleExpressionUpdate, handleBatchExpressionsUpdate);
    }
    else {
...
exports.mcpTools = {
    setAvatarExpression: {
        ...
        function: async (name, direction, posX, posY, rotation, scale) => {
    ...
    setBatchExpressions: {
        ...
        function: async (loop, random, actions) => {
